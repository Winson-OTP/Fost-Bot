...
function addChannel(guildID, channelID, cb) {
    client.connect(err => {
        const collection = client.db("Channel").collection("command-channel");
        collection.insertOne({ guildid: guildID, channelid: channelID });
        cb()
    });
}
...
module.exports = {
    addChannel, deleteChannel, searchByGuildID, searchByChannelID
}